...
class RealEnv:
    ...
    def step(self, action, base_action=None, get_tracer_vel=False, get_obs=True):
        ...
        if not self.qwer and self.prev_gripper > 50:
            self.qwer = True
            
            # 한번 명령 내리면 자체적으로 리니어 모터가 작동시작하며 소프트 그리퍼에 공압이 작동함.
            # 신호A 받으면 바람을 넣는 방향으로 리니어 동작.
            # 신호B 받으면 바람 빼는 방향으로 리니어 동작. 


            self.plc.plc_motor_ctrl("05")  # "05"는 신호 A를 의미
            
        if self.qwer and self.prev_gripper < 50:
            self.qwer = False

            self.plc.plc_motor_ctrl("06")  # "06"는 신호 B를 의미
